...
def run_train(args, train_data_loader, net, optimizer, epoch, iteration):
    ...
    for internel_iter, (images, gt_boxes, gt_labels, ego_labels, counts, img_indexs, wh) in enumerate(train_data_loader):
        ...
        # Check per loss esplosa
        if not torch.isfinite(loss_l) or not torch.isfinite(loss_c):
            logger.warning(f"[LOSS NAN/INF] Iteration {iteration}, Sample indices: {img_indexs}")
            logger.warning(f"  > Location loss: {loss_l}, Classification loss: {loss_c}")
            logger.warning(f"  > gt_boxes shape: {gt_boxes.shape}, labels: {gt_labels}")
            logger.warning(f"  > Counts: {counts}, ego_labels: {ego_labels}")
